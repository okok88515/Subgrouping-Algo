...
int CQI = 15;
int NumUe =50 ;
string inCQIGroup("CQIGroups150vc.txt");
string inRBCQI("RBCQIs150vc.txt");
string inUserID("UserID150vc.txt");
int QualityLevel=5;
...
vector<int> FindProperGroupConfig(){
	...
	while(CurrentUtility>MaxUtility){
		bestCand=GroupConfigList[bestCandIndex];
		NumOfSplitGroups++;
		if(NumOfSplitGroups==3)break;
		GroupConfig.clear();
		SDConfig.clear();
		CandidateNo.clear();
		NumOfCand=0;
		...
		for(int i =0; i<configsize;++i){
            for(int j=0;j<GroupConfigList[i].size();++j){
